...
// Registration disabled - Route::get('/register/{lang?}', [RegisteredUserController::class, 'showRegistrationForm'])->name('register');
...
Route::get('/home', [DashboardController::class, 'account_dashboard_index'])->name('home')->middleware(['XSS', 'revalidate', 'auth']);
// Route::get('/dashboard', [DashboardController::class, 'account_dashboard_index'])->name('dashboard')->middleware(['XSS', 'revalidate', 'auth']);
...
// Registration disabled - Route::post('register', [RegisteredUserController::class, 'store'])->name('register');
   ...
   Route::get('users', [UserController::class, 'index'])->name('users.index')->middleware(['auth', 'XSS', 'revalidate']);
    ...
    Route::group(
        [
            ...
            // Add this to web.php
Route::get('/fix-company-role-perms', function() {
    $role = \Spatie\Permission\Models\Role::where('name', 'company')->first();
    
    if (!$role) {
        return 'Company role not found!';
    }
    ...
    // Also assign to user ID 2 (your user) directly for testing
    ...
    return response()->json([
        'message' => 'Permissions assigned to company role and user!',
        'role' => $role->name,
        'assigned_permissions' => $permissions,
        'user_id' => $user ? $user->id : 'Not found',
    ]);
})->middleware('auth');
            // Add this to web.php
Route::get('/check-assigned-perms', function() {
    $user = \Auth::user();
    ...
    return response()->json([
        ...
        'permissions_in_db' => \Spatie\Permission\Models\Permission::where('name', 'like', 'enquiry%')->pluck('name'),
        
        // Check role permissions
        'role_permissions' => $role ? $role->permissions->whereIn('name', ['enquiry_view', 'enquiry_add', 'enquiry_edit', 'enquiry_delete'])->pluck('name') : [],
        
        // Direct user permissions
        'user_direct_permissions' => $user->getDirectPermissions()->whereIn('name', ['enquiry_view', 'enquiry_add', 'enquiry_edit', 'enquiry_delete'])->pluck('name'),
        
        // Test each permission
        'can_enquiry_view' => $user->can('enquiry_view'),
        'can_enquiry_add' => $user->can('enquiry_add'),
        'can_enquiry_edit' => $user->can('enquiry_edit'),
        'can_enquiry_delete' => $user->can('enquiry_delete'),
    ]);
})->middleware('auth');
...
Route::get('/test-enquiry-no-middleware', function() {
    return "Direct access - If you see this, middleware is blocking enquiry routes";
});
    Route::get('/debug-company-enquiry-permissions', function() {
    $user = auth()->user();
    ...
    $hasCompanyRole = $user->hasRole('company');
    
    // Check specific permissions directly
    $permissionsCheck = [
        'Company Permissions' => [
            'company_view' => $user->can('company_view'),
            'company_add' => $user->can('company_add'),
            'company_edit' => $user->can('company_edit'),
            'company_delete' => $user->can('company_delete'),
        ],
        'Enquiry Permissions' => [
            'enquiry_view' => $user->can('enquiry_view'),
            'enquiry_add' => $user->can('enquiry_add'),
            'enquiry_edit' => $user->can('enquiry_edit'),
            'enquiry_delete' => $user->can('enquiry_delete'),
            'enquiry_report' => $user->can('enquiry_report'),
        ]
    ];
    
    // Check via hasPermissionTo (more accurate)
    $hasPermissionToCheck = [
        'Company (hasPermissionTo)' => [
            'company_view' => $user->hasPermissionTo('company_view'),
            'company_add' => $user->hasPermissionTo('company_add'),
            'company_edit' => $user->hasPermissionTo('company_edit'),
            'company_delete' => $user->hasPermissionTo('company_delete'),
        ],
        'Enquiry (hasPermissionTo)' => [
            'enquiry_view' => $user->hasPermissionTo('enquiry_view'),
            'enquiry_add' => $user->hasPermissionTo('enquiry_add'),
            'enquiry_edit' => $user->hasPermissionTo('enquiry_edit'),
            'enquiry_delete' => $user->hasPermissionTo('enquiry_delete'),
            'enquiry_report' => $user->hasPermissionTo('enquiry_report'),
        ]
    ];
    
    // Check if permissions exist in database
    $permissionsInDb = \Spatie\Permission\Models\Permission::where('name', 'like', 'company_%')
        ->orWhere('name', 'like', 'enquiry_%')
        ->get()
        ->pluck('name');
    
    // Check if company role has these permissions
    $companyRolePermissions = [];
    if ($companyRole) {
        $companyRolePermissions = $companyRole->permissions
            ->whereIn('name', ['company_view', 'company_add', 'company_edit', 'company_delete', 
                             'enquiry_view', 'enquiry_add', 'enquiry_edit', 'enquiry_delete', 'enquiry_report'])
            ->pluck('name');
    }
    ...
    return response()->json([
        'user_info' => [
            ...
            'has_company_role' => $hasCompanyRole,
        ],
        
        'permissions_check_can_method' => $permissionsCheck,
        'permissions_check_hasPermissionTo_method' => $hasPermissionToCheck,
        
        'database_info' => [
            'permissions_in_db' => $permissionsInDb,
            'company_role_permissions' => $companyRolePermissions,
            'user_direct_permissions' => $userDirectPermissions,
        ],
        
        'debug_actions' => [
            'assign_company_role_url' => url('/assign-company-role-to-user'),
            'assign_permissions_directly_url' => url('/assign-direct-permissions'),
            'clear_cache_url' => url('/clear-cache-permissions'),
        ]
    ]);
})->middleware(['auth']);


    // Add this route to assign all permissions to company role
Route::get('/assign-all-permissions-to-company', function() {
    $companyRole = \Spatie\Permission\Models\Role::where('name', 'company')->first();
    ...
    // Get all permissions from the seeder
    $permissions = [
        // Enquiry permissions
        'enquiry_view',
        'enquiry_add', 
    ...
    // Assign all permissions to company role
    ...
    return response()->json([
        'success' => true,
        'message' => 'All permissions assigned to company role!',
        'assigned_permissions' => $companyRole->permissions->pluck('name')
    ]);
})->middleware(['auth']);
    ...
    // NOTE: Company routes have been MOVED OUTSIDE the verified group below
...
//    Route::get('/bill/{id}/', 'PurchaseController@purchaseLink')->name('purchase.link.copy');
    ...
    Route::put('enquiry/{id}/complete', [EnquiryController::class, 'complete'])->name('enquiry.complete');

   // web.php
Route::get('enquiry/{id}/convert-to-quotation', [EnquiryController::class, 'convertToQuotation'])
    ->name('enquiry.convertToQuotation');
 // NOTE: Enquiry routes have been MOVED OUTSIDE the verified group below

// Route::get('enquiry/export/csv', [EnquiryController::class, 'exportCsv'])->name('enquiry.export.csv');
// Route::get('enquiry/print', [EnquiryController::class, 'print'])->name('enquiry.print');
// Enquiry routes
Route::get('enquiry/export/csv', [EnquiryController::class, 'exportCsv'])->name('enquiry.export.csv');
Route::get('enquiry/print', [EnquiryController::class, 'print'])->name('enquiry.print');
...
// Category Routes - Minimal CRUD
Route::get('categories', [CategoryController::class, 'index'])->name('categories.index'); // View list
Route::get('categories/create', [CategoryController::class, 'create'])->name('categories.create'); // Add form
Route::post('categories', [CategoryController::class, 'store'])->name('categories.store'); // Save new
Route::get('categories/{category}', [CategoryController::class, 'show'])->name('categories.show'); // View single
Route::get('categories/{category}/edit', [CategoryController::class, 'edit'])->name('categories.edit'); // Edit form
Route::put('categories/{category}', [CategoryController::class, 'update'])->name('categories.update'); // Update
Route::delete('categories/{category}', [CategoryController::class, 'destroy'])->name('categories.destroy'); // Delete



Route::get('/assign-variant-permissions-to-company', function() {
    ...
    // Assign variant permissions to company role
    ...
    return response()->json([
        'success' => true,
        'message' => 'Variant permissions assigned to company role!',
        'assigned_permissions' => $companyRole->permissions()->where('name', 'like', 'variant_%')->pluck('name'),
        'user_now_has_access' => auth()->check() ? auth()->user()->can('variant_view') : 'Not logged in'
    ]);
})->middleware(['auth']);

// Add this to your debug route
Route::get('/debug-db-permissions', function() {
    // Check if variant permissions exist in permissions table
    $permissions = \Spatie\Permission\Models\Permission::where('name', 'like', 'variant_%')->get();
    
    // Check if company role exists
    $companyRole = \Spatie\Permission\Models\Role::where('name', 'company')->first();
    
    // Check what permissions the company role has
    if ($companyRole) {
        $rolePermissions = $companyRole->permissions->pluck('name');
    }
    
    return response()->json([
        'variant_permissions_in_db' => $permissions->pluck('name'),
        'company_role_exists' => $companyRole ? 'Yes' : 'No',
        'company_role_permissions' => $rolePermissions ?? [],
    ]);
});
...
Route::get('/debug-permissions', function() {
    ...
    $permissions = [
        'variant_view' => $user->can('variant_view'),
        'variant_add' => $user->can('variant_add'),
        'variant_edit' => $user->can('variant_edit'),
        'variant_delete' => $user->can('variant_delete'),
    ];
...
// Variants Routes - Minimal CRUD
Route::get('item-variants', [ItemVariantController::class, 'index'])->name('item-variants.index'); // View list
Route::get('item-variants/create', [ItemVariantController::class, 'create'])->name('item-variants.create'); // Add form
Route::post('item-variants', [ItemVariantController::class, 'store'])->name('item-variants.store'); // Save new
Route::get('item-variants/{item_variant}', [ItemVariantController::class, 'show'])->name('item-variants.show'); // View single
Route::get('item-variants/{item_variant}/edit', [ItemVariantController::class, 'edit'])->name('item-variants.edit'); // Edit form
Route::put('item-variants/{item_variant}', [ItemVariantController::class, 'update'])->name('item-variants.update'); // Update
Route::delete('item-variants/{item_variant}', [ItemVariantController::class, 'destroy'])->name('item-variants.destroy'); // Delete
    //joining Letter
...
// ====================================================
// CRITICAL ROUTES MOVED OUTSIDE VERIFIED GROUP
// ====================================================

// Company Routes (accessible without email verification)
...
// Enquiry Routes (accessible without email verification)
...
Route::get('enquiry/{id}/convert-to-quotation', [EnquiryController::class, 'convertToQuotation'])->name('enquiry.convertToQuotation');
Route::get('enquiry/export/csv', [EnquiryController::class, 'exportCsv'])->name('enquiry.export.csv');
Route::get('enquiry/print', [EnquiryController::class, 'print'])->name('enquiry.print');

// Helper routes for debugging and fixing permissions
Route::get('/assign-company-role-to-user', function() {
    $user = auth()->user();
    ...
    return response()->json([
        'success' => true,
        'message' => 'Company role assigned to user!',
        'user_id' => $user->id,
        'user_name' => $user->name,
        'assigned_role' => 'company',
        'user_roles_now' => $user->getRoleNames()
    ]);
})->middleware(['auth']);

Route::get('/assign-direct-permissions', function() {
    $user = auth()->user();
    ...
    return response()->json([
        ...
        'user_id' => $user->id,
        'assigned_permissions' => $permissions,
        'user_has_enquiry_view_now' => $user->hasPermissionTo('enquiry_view'),
        'user_has_company_view_now' => $user->hasPermissionTo('company_view'),
    ]);
})->middleware(['auth']);

Route::get('/clear-cache-permissions', function() {
    \Illuminate\Support\Facades\Artisan::call('cache:clear');
    ...
    return response()->json([
        'success' => true,
        'message' => 'Cache cleared! Please log out and log back in.',
    ]);
})->middleware(['auth']);
