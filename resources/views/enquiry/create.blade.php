        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title">{{__('Enquiry')}}</h5>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('enquiry.store') }}" enctype="multipart/form-data" id="enquiryForm">
                        @csrf
                        ...
                        <div class="row mb-4">
                           <div class="col-md-6">
                                <div class="form-group">
                                    <label for="enquiry_no" class="form-label">{{ __('Enquiry No') }} <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" id="enquiry_no" value="{{ old('enquiry_no', $suggestedSerial) }}" name="enquiry_no" required>
                                    <small class="form-text text-muted">Enter your custom enquiry number</small>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="enquiry_date" class="form-label">{{ __('Date') }} <span class="text-danger">*</span></label>
                                    <input type="date" class="form-control" id="enquiry_date" name="enquiry_date" value="{{ old('enquiry_date', date('Y-m-d')) }}" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for="company_id" class="form-label">{{ __('Company Name') }} <span class="text-danger">*</span></label>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="kind_attn" class="form-label">{{ __('Kind Attn.') }}</label>
                                    <input type="text" class="form-control" id="kind_attn" name="kind_attn" value="{{ old('kind_attn') }}" placeholder="{{ __('Person Name') }}">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="mail_id" class="form-label">{{ __('Mail-ID') }}</label>
                                    <input type="email" class="form-control" id="mail_id" name="mail_id" value="{{ old('mail_id') }}" placeholder="{{ __('Email') }}">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="phone_no" class="form-label">{{ __('Phone No') }}</label>
                                    <input type="text" class="form-control" id="phone_no" name="phone_no" value="{{ old('phone_no') }}" placeholder="{{ __('Phone Number') }}">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" id="remarks" name="remarks" rows="2" placeholder="{{ __('Additional remarks...') }}">{{ old('remarks') }}</textarea>
                                </div>
                            </div>
                        </div>
...
<div id="items-container">
    ...
    <div class="item-row mb-4 border p-3 rounded" data-item-index="1">
        <div class="row mb-3">
            <div class="col-md-10">
                <h6 class="text-primary mb-0">{{ __('Item') }} <span class="item-number">1</span></h6>
            </div>
            <div class="col-md-2 text-end">
                <button type="button" class="btn btn-sm btn-danger remove-item-btn" data-index="1">
                    <i class="ti ti-minus"></i> {{ __('Remove') }}
                </button>
            </div>
        </div>
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label for="item_select_1" class="form-label">{{ __('Select Item') }} <span class="text-danger">*</span></label>
                    <select class="form-control item-select select2-item" id="item_select_1" name="items[1][item_id]" data-index="1">
                        ...
                        @foreach($items as $item)
                            <option value="{{ $item->id }}" 
                                   ...
                                   data-image-url="{{ $item->image ? Storage::url($item->image) : '' }}"
            ...
            <div class="col-md-2">
                <div class="form-group">
                    <label for="item_qty_1" class="form-label">{{ __('Qty') }}</label>
                    <input type="number" class="form-control item-qty" id="item_qty_1" name="items[1][quantity]" value="{{ old('items.1.quantity', 1) }}" min="1" placeholder="Qty" required>
                </div>
            </div>
            <div class="col-md-4">
                <div class="form-group">
                    <label for="item_image_1" class="form-label">{{ __('Image') }}</label>
                    <div class="input-group">
                        ...
                        <button type="button" class="btn btn-outline-primary pick-image-btn" data-target="item_image_1">
                            <i class="ti ti-plus"></i> {{ __('Pick Image') }}
                        </button>
                    </div>
                    ...
                    <div class="mt-2" id="imagePreviewArea_1">
                        ...
                        <input type="hidden" name="salesman_id" value="{{ Auth::user()->hasRole('Sales Engineer') ? Auth::id() : '' }}">
        ...
        $('#add-item-btn').click(function() {
            ...
            const newItem = `
                <div class="item-row mb-4 border p-3 rounded" data-item-index="${itemCount}">
                    <div class="row mb-3">
                        <div class="col-md-10">
                            <h6 class="text-primary mb-0">{{ __('Item') }} <span class="item-number">${itemCount}</span></h6>
                        </div>
                        <div class="col-md-2 text-end">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="item_select_${itemCount}" class="form-label">{{ __('Select Item') }} <span class="text-danger">*</span></label>
                            ...
                            <div class="form-group mt-3">
                                <label for="item_description_${itemCount}" class="form-label">{{ __('Item Description') }} <span class="text-danger">*</span></label>
                                <textarea class="form-control item-description" id="item_description_${itemCount}" name="items[${itemCount}][description]" rows="3" placeholder="{{ __('Item description...') }}" required></textarea>
                                <small class="text-muted">Description will auto-fill when you select an item</small>
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div class="form-group">
                                <label for="item_qty_${itemCount}" class="form-label">{{ __('Qty') }}</label>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="item_image_${itemCount}" class="form-label">{{ __('Image') }}</label>
                                <div class="input-group">
