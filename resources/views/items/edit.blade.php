...
@section('content')
 @php
    ...
    <div class="form-group">
        ...
        <input type="hidden" name="tax_percentage" id="selected_tax_percentage" 
       value="{{ old('tax_percentage', $item->tax ? $item->tax->rate : 0) }}">
<input type="hidden" name="tax_name" id="selected_tax_name" 
       value="{{ old('tax_name', $item->tax ? $item->tax->name : '') }}">
<input type="hidden" name="tax_id" id="selected_tax_id" 
       value="{{ old('tax_id', $item->tax ? $item->tax->id : '') }}">
...
<script>
    ...
    // Pass the current item's tax data
    const currentItemTax = @json($item->tax ?? null);
    
    $(document).ready(function() {
        // Load existing tax percentages from database
        loadExistingTaxPercentages(existingTaxes, currentItemTax);
        ...
        // Handle form validation errors - reselect tax if validation fails
        handleValidationErrors();
    });
    
    // Function to handle form validation errors
    function handleValidationErrors() {
        // Check if there are validation errors for tax fields
        const oldTaxPercentage = parseFloat('{{ old("tax_percentage", 0) }}') || 0;
        const oldTaxName = '{{ old("tax_name", "") }}';
        const oldTaxId = '{{ old("tax_id", "") }}';
        
        // If there's old tax data from validation errors, use that
        if (oldTaxPercentage > 0 || oldTaxName) {
            setTimeout(function() {
                selectTaxPercentage(oldTaxPercentage, oldTaxName, oldTaxId);
            }, 500);
        } else if (currentItemTax) {
            // Otherwise use the current item's tax
            setTimeout(function() {
                selectTaxPercentage(
                    currentItemTax.rate,
                    currentItemTax.name,
    ...
    // Function to select a tax percentage in the dropdown
    function selectTaxPercentage(percentage, name, id) {
        const taxPercent = parseFloat(percentage) || 0;
        const taxName = name || 'TAX@' + taxPercent + '%';
        const taxId = id || '';
        const value = taxPercent.toString();
        
        // Check if this tax exists in our array
        let existingTax = customTaxPercentages.find(tax => 
            tax.id === taxId || tax.percentage === taxPercent || tax.name === taxName
        );
        
        if (!existingTax && taxPercent > 0) {
            // Add the tax to our array
            const displayName = taxName.includes('TAX@') ? taxName : 'TAX@' + taxPercent + '%';
            const newTaxId = taxId || 'temp_' + Date.now();
            ...
            existingTax = customTaxPercentages.find(tax => tax.id === newTaxId);
        }
        ...
        if (existingTax) {
            ...
            $('#selected_tax_percentage').val(taxPercent);
            $('#selected_tax_name').val(taxName);
            $('#selected_tax_id').val(taxId || existingTax.id);
        }
    }
    
    // Function to load existing tax percentages from database
    function loadExistingTaxPercentages(taxes, currentItemTax) {
        ...
        // Add current item's tax if it exists and isn't already in the array
        if (currentItemTax) {
            const existingTax = customTaxPercentages.find(tax => tax.id == currentItemTax.id);
            if (!existingTax) {
                // Check if tax name already has TAX@ prefix
                let displayName = currentItemTax.name;
                if (!displayName.includes('TAX@')) {
                    displayName = 'TAX@' + currentItemTax.rate + '%';
                }
                
                customTaxPercentages.push({
                    id: currentItemTax.id,
                    percentage: parseFloat(currentItemTax.rate),
                    name: currentItemTax.name,
                    display: displayName,
                    value: currentItemTax.rate.toString()
                });
            }
        }
