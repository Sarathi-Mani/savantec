...

{
    ...
    public function getProfileAttribute()
    {
        ...
        {
            // Check if avatar exists in public storage (new location)
            if(\Storage::disk('public')->exists($this->avatar))
        {
            return $this->attributes['avatar'] = asset(\Storage::url($this->avatar));
            }
            // Fallback: check old storage location
            elseif(file_exists(storage_path('uploads/avatar/' . $this->avatar)))
            {
                // Try to move file to new location for future use
                $oldPath = storage_path('uploads/avatar/' . $this->avatar);
                $newPath = storage_path('app/public/uploads/avatar/' . $this->avatar);
                if(!file_exists(dirname($newPath)))
                {
                    @mkdir(dirname($newPath), 0755, true);
                }
                @copy($oldPath, $newPath);
                return $this->attributes['avatar'] = asset(\Storage::url('uploads/avatar/' . $this->avatar));
            }
            // If avatar path exists but file doesn't, return default
        }
        
        // Return default avatar
        // Check if default avatar exists in public storage
        if(\Storage::disk('public')->exists('uploads/avatar/avatar.png'))
        {
            return $this->attributes['avatar'] = asset(\Storage::url('uploads/avatar/avatar.png'));
        }
        elseif(\Storage::disk('public')->exists('uploads/avatar/default.png'))
        {
            return $this->attributes['avatar'] = asset(\Storage::url('uploads/avatar/default.png'));
        }
        elseif(file_exists(storage_path('uploads/avatar/avatar.png')))
        {
            // Copy default avatar to new location
            $oldPath = storage_path('uploads/avatar/avatar.png');
            $newPath = storage_path('app/public/uploads/avatar/avatar.png');
            if(!file_exists(dirname($newPath)))
            {
                @mkdir(dirname($newPath), 0755, true);
            }
        ...
        {
            return $this->attributes['avatar'] = asset('assets/images/user/avatar.png');
        }
    }
    ...
    public function getImgImageAttribute()
    {
        ...
        if(!empty($userDetail))
        {
            ...
            else
            {
                ...
                elseif(\Storage::disk('public')->exists('uploads/avatar/default.png'))
                {
                    return asset(\Storage::url('uploads/avatar/default.png'));
        }
        else
        {
    ...
    /**
     * Get the first accessible menu route for the user
     * Returns the first page the user has permission to access
     * 
     * @return string|null
     */
    public function getFirstAccessibleRoute()
    {
        // Check dashboards first (in order of appearance)
        // Note: Also check if the module is enabled in the user's plan
        if($this->can('show account dashboard') && self::show_account() == 1) {
            return route('dashboard');
        }
        if($this->can('show report dashboard') && self::show_report() == 1) {
            return route('report.dashboard');
        }
        if($this->can('show hrm dashboard') && self::show_hrm() == 1) {
            return route('hrm.dashboard');
        }
        if($this->can('show crm dashboard') && self::show_crm() == 1) {
            return route('crm.dashboard');
        }
        if($this->can('show project dashboard') && self::show_project() == 1) {
            return route('project.dashboard');
        }
        if($this->can('show pos dashboard') && self::show_pos() == 1) {
            return route('pos.dashboard');
        }

        // Check CRM System (only if CRM module is enabled)
        if(self::show_report() == 1) {
            if($this->can('Enquiry Report')) {
                return route('enquiry.index');
            }
            if($this->can('manage deal')) {
                return route('deals.index');
            }
        }


        // Check HRM System items
        if(self::show_hrm() == 1) {
            if($this->can('manage employee')) {
                return route('employee.index');
            }
            if($this->can('manage set salary')) {
                return route('setsalary.index');
            }
            if($this->can('manage pay slip')) {
                return route('payslip.index');
            }
            if($this->can('manage leave')) {
                return route('leave.index');
            }
            if($this->can('manage attendance')) {
                return route('attendanceemployee.index');
            }
        }

        // Check Project System (only if project module is enabled)
        if(self::show_project() == 1) {
            if($this->can('manage project')) {
                return route('projects.index');
            }
            if($this->can('manage project task')) {
                return route('taskBoard.view', 'list');
            }
            if($this->can('manage timesheet')) {
                return route('timesheet.list');
            }
            if($this->can('manage bug report')) {
                return route('bugs.view', 'list');
            }
        }

        // Check CRM System (only if CRM module is enabled)
        if(self::show_crm() == 1) {
            if($this->can('manage lead')) {
        ...
        if(self::show_account() == 1) {
            // Check in order of menu appearance
            if($this->can('manage customer')) {
                return route('customer.index');
            }
            if($this->can('manage vender')) {
                return route('vender.index');
            }
            if($this->can('manage proposal')) {
                return route('proposal.index');
            }
            if($this->can('create bank account') || $this->can('manage bank account')) {
                return route('bank-account.index');
            }
            if($this->can('manage invoice')) {
                return route('invoice.index');
            }
            if($this->can('manage revenue')) {
                return route('revenue.index');
            }
            if($this->can('manage bill')) {
                return route('bill.index');
            }
            if($this->can('manage payment')) {
                return route('payment.index');
            }
            if($this->can('manage product & service')) {
                return route('productservice.index');
            }
        }

        // Check POS System (only if POS module is enabled)
        if(self::show_pos() == 1) {
            if($this->can('manage pos')) {
        ...
        if($this->can('manage role')) {
            return route('roles.index');
        }
        if($this->can('manage client')) {
            return route('clients.index');
        }

        // If nothing is accessible, return null (will redirect to HRM dashboard as fallback)
        return null;
    }
